...
const resetDatabase = async () => {
    console.log('🗑️  Memulai reset database...\n');
    ...
    try {
        ...
        for (const table of tables) {
            try {
                ...
                console.log(`⚠ Tabel ${table} tidak ada atau error: ${err.message}`);
            }
        }
        ...
        console.log('\n✓ User 1 created: user1@test.com / user123');

        await pool.query(
            `INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, ?)`,
            ['user2', 'user2@test.com', userPassword, 'user']
        );
        console.log('✓ User 2 created: user2@test.com / user123');

        await pool.query(
            `INSERT INTO users (username, email, password, role) VALUES (?, ?, ?, ?)`,
            ['user3', 'user3@test.com', userPassword, 'user']
        );
        console.log('✓ User 3 created: user3@test.com / user123');
